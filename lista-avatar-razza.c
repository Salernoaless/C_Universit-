...
void inserisciAvatar(struct sAvatar *listaAvatar,int *dimensioneAttuale);
void ricercaAvatar(const struct sAvatar *listaAvatar,int dimensioneAttuale, char nomeDaStampare[]);
void stampaListaAvatar(const struct sAvatar *listaAvatar,int dimensioneAttuale);
void cancellaAvatar(struct sAvatar *listaAvatar,int *dimensioneAttuale,char nomeCancellare[]);
void rimuoviNewline(char *stringa);
int leggiInteroLimitato(const char *richiesta,int min, int max);
int trovaIndice(const struct sAvatar *listaAvatar, int dimensioneAttuale, char nomeCercato[]);
...
void stampaListaAvatar(const struct sAvatar *listaAvatar,int dimensioneAttuale){
    char *nomiRazze[] = {"Umano","Elfo","Nano","Orco"};
    ...
    for(int i = 0; i < dimensioneAttuale;i++){
        printf("%s (%s), salute:%d, forza:%d, magia: %d\n",(listaAvatar+i)->nome,nomiRazze[(listaAvatar+i)->idRazza],(listaAvatar+i)->salute,(listaAvatar+i)->forza,(listaAvatar+i)->magia);
    }

    return;
}

void ricercaAvatar(const struct sAvatar *listaAvatar,int dimensioneAttuale, char nomeCercato[]){
    char *nomiRazze[] = {"Umano","Elfo","Nano","Orco"};
    int posizione = trovaIndice(listaAvatar,dimensioneAttuale,nomeCercato);
    if( posizione != -1){
        printf("%s (%s), salute:%d, forza:%d, magia: %d\n",(listaAvatar+posizione)->nome,nomiRazze[(listaAvatar+posizione)->idRazza],(listaAvatar+posizione)->salute,(listaAvatar+posizione)->forza,(listaAvatar+posizione)->magia);
        return;
    }
}
